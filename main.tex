...
\]
We will use mean value theorem and observe that $\frac{d}{dx}\ln(1+x^2) = \frac{2x}{1+x^2} $ where $|\frac{2x}{1+x^2}| \leq 1$. Thus we have that
\[
|\ln(1+x^2)-\ln(1+y^2)|=|f'(c)(x-y)|\leq 1 |x-y|
\]
Which means that we can take $C=D=2$ so we can conclude by the Theorem $5.2.1$ that the solution is adapted to $\mathcal{F}^Z_t$ generated by $Z$ and $B_s$. This means that it is a strong solution.
\end{solution}
